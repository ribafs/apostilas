...
if(isset($_REQUEST['btn_login']))
{
	...
	if(empty($email)){						
		...
		try
		{
			...
			$select_stmt=$db->prepare("SELECT email,password,role,enabled FROM users
										WHERE
										email=:uemail");
			...
			if($email!=null AND $password!=null)	//check taken fields not null after countinue
			{
				...
				{
					if($email==$dbemail && ($dbenabled == true))
					{
                ...
                // Acesso de acordo com a role do user
						    switch($dbrole)		//role base user login start
						    {
							    case "super":
								    ...
								    header("refresh:3;admin/users/users/index.php");	//refresh 3 second after redirect to "admin_home.php" page
								    break;
